                                ...
                                <div className="col-12">
                                    <table className="table mt-3">
                                        ...
                                        {modal?.item?.orderItems?.map(item => (
                                            <tr key={item?.id} style={{margin:'6px 0',cursor:'pointer'}} onClick={() => window.open(process.env.REACT_APP_MAIN_SITE_URL + `product/${item?.productDetail?.product?.id}?productDetailId=${item?.productDetail?.id}`)}>
